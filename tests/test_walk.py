...
def test_walk_collect_returns_entries(sample_tree):
    entries = fs_watcher.walk_collect(str(sample_tree))
...
def test_walk_collect_files_only(sample_tree):
    entries = fs_watcher.walk_collect(str(sample_tree), file_type="file")
...
def test_walk_collect_dirs_only(sample_tree):
    entries = fs_watcher.walk_collect(str(sample_tree), file_type="dir")
    assert all(e.is_dir for e in entries)
    assert all(not e.is_file for e in entries)


def test_walk_max_depth(sample_tree):
    entries = fs_watcher.walk_collect(str(sample_tree), max_depth=1)
    assert all(e.depth <= 1 for e in entries)


def test_walk_glob_pattern(sample_tree):
    entries = fs_watcher.walk_collect(str(sample_tree), glob_pattern="*.txt")
    assert all(e.path.endswith(".txt") for e in entries)


def test_walk_sorted(sample_tree):
    entries = fs_watcher.walk_collect(str(sample_tree), sort=True, file_type="file")
    paths = [e.path for e in entries]
    assert paths == sorted(paths)


def test_walk_nonexistent_raises():
    with __import__("pytest").raises(fs_watcher.WalkError):
        fs_watcher.walk_collect("/nonexistent/path/xyz")


def test_walk_iterator(sample_tree):
    """Test the streaming walk() iterator."""
    count = 0
    for entry in fs_watcher.walk(str(sample_tree), file_type="file"):
...
def test_walk_matches_os_walk(sample_tree):
    ...
    os_files = set()
    for root, dirs, files in os.walk(str(sample_tree)):
        for f in files:
            os_files.add(os.path.join(root, f))

    fw_entries = fs_watcher.walk_collect(str(sample_tree), file_type="file")
